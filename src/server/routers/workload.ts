      ...
      if (allResults.length > 0) {
        const rows = allResults
          ...
          .map(r => ({
            ...
            demand_volume: Math.min(r.demand_volume, 9999999999),
            conversion_ratio: Math.min(r.conversion_ratio, 9999.9999),
            process_volume: Math.min(r.process_volume, 9999999999),
            weighted_uph: r.weighted_uph != null ? Math.min(r.weighted_uph, 9999999999) : null,
            hours_needed: Math.min(r.hours_needed ?? 0, 9999999999),
            fte_needed: Math.min(r.fte_needed ?? 0, 99999999),
            hours_assigned: Math.min(r.hours_available ?? 0, 9999999999),
            fte_assigned: Math.min(r.fte_available ?? 0, 99999999),
            coverage_pct: Math.min(r.coverage_pct, 999),
