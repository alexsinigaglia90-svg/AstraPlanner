...
## AstraPlanner — Template voor verwerkingsverantwoordelijken

**Versie:** 1.0 — opgesteld door AstraPlanner als hulpmiddel voor klanten
**Doel:** Dit template stelt een verwerkingsverantwoordelijke in staat om — met beperkte aanvullende informatie die door AstraPlanner wordt geleverd — zelfstandig een Data Protection Impact Assessment uit te voeren conform artikel 35 AVG.
...
**Onze aanbeveling:** omdat AstraPlanner meestal twee of drie van deze criteria raakt, adviseren wij elke verwerkingsverantwoordelijke om een DPIA uit te voeren voordat het platform in productie wordt genomen. Dit template versnelt dat proces.

### 1.3 Hoe dit template te gebruiken

Elk hoofdstuk bevat:
- **Input van AstraPlanner** — vooraf ingevulde feitelijke informatie, gebaseerd op het Security Document en de Verwerkersovereenkomst.
...
**Input van AstraPlanner:**
AstraPlanner is een multi-tenant SaaS-platform voor personeelsplanning. De verwerking omvat:
...
| Vraag | Antwoord |
|---|---|
| Welke AstraPlanner-modules gebruiken jullie actief? | `[INVULLEN: bv. planning, verzuim, AI-assistent]` |
...
**Input van AstraPlanner:**
De standaardbewaartermijnen zijn vastgelegd in §4.4 van het Security Document. Medewerkergegevens worden bewaard gedurende de arbeidsrelatie plus een configureerbare nabewaartermijn. Na beëindiging van de overeenkomst verwijdert AstraPlanner alle data binnen 30 dagen, met uitzondering van back-ups die automatisch aflopen binnen 7 dagen.
...
Voor elk risico scoor je waarschijnlijkheid (1-4) en impact (1-4). Het residuele risico is het risico dat resteert na de reeds getroffen maatregelen van AstraPlanner.

### Risico 1 — Ongeautoriseerde toegang door een andere AstraPlanner-klant

**Bron:** gedeeld multi-tenant platform.

**Mitigatie door AstraPlanner (reeds aanwezig):** Row-Level Security op database-niveau plus expliciete `organization_id`-filter in de applicatielaag — defense-in-depth. Geen cross-tenant data-toegang mogelijk zonder tegelijk meerdere onafhankelijke lagen te doorbreken. Zie §6 Security Document.
...
### Risico 2 — Ongeautoriseerde toegang door AstraPlanner-personeel

**Bron:** de verwerker heeft technisch de mogelijkheid om via een service-role sleutel RLS te omzeilen.

**Mitigatie door AstraPlanner:** geen routinematige toegang door personeel; service-role sleutel is uitsluitend als versleutelde omgevingsvariabele aanwezig in productieomgeving; toegang wordt onveranderbaar gelogd; contractuele beperkingen in de Verwerkersovereenkomst. Zie §6.6 Security Document.

| Score | Waarde |
|---|---|
| Waarschijnlijkheid | 1 (zeer laag — vereist kwade opzet van een bevoegde AstraPlanner-medewerker) |
| Impact | 4 (zeer hoog) |
...
**Mitigatie door AstraPlanner:** Zod-validatie op alle invoer, geparametriseerde queries (geen SQL-injectie), React-escaping (geen XSS), server-side JWT-verificatie per request, rate limiting, HTTPS-only met HSTS + CSP headers, geautomatiseerde dependency-scanning, externe pen-test vóór productie-uitrol. Zie §§7-9 en §14 Security Document.
...
**Mitigatie door AstraPlanner:** pseudonimisering van direct identificerende velden vóór verzending; Standard Contractual Clauses Module 2 via Anthropic's DPA; aggregaten in plaats van individuele data bij de Insights-functionaliteit. Zie §11 Security Document.
...
**Mitigatie door AstraPlanner:** directe UI-knoppen voor `tenant_admins` om alle betrokkenenrechten zelfstandig af te handelen, inclusief AVG-art. 17 recht op vergetelheid met dubbele audit-trail. Zie §12.2 Security Document.
...
**Mitigatie door AstraPlanner:** elk advies is adviserend van aard; elke wijziging in dienstroosters of medewerkerdata vereist een handmatige bevestiging door een bevoegde gebruiker. Artikel 22 AVG is daarmee niet van toepassing. Zie §11.5 Security Document.
...
**Mitigatie door AstraPlanner:** directe erasure-UI voor `tenant_admins`; `status = 'terminated'` als archiveer-modus; `deleted_at` + `deleted_by` markeringen; automatische opruimroutine voor contact-formulier-submissies na 1 jaar.
...
**Onze beoordeling voor AstraPlanner:** gegeven de in §4 van deze DPIA geïdentificeerde residuele risico's (allemaal laag tot middel) is voorafgaande raadpleging doorgaans **niet** vereist. Bevestig dit echter zelf op basis van je specifieke implementatie.
...
> **Support:** Indien je vragen hebt bij het invullen van deze DPIA, neem dan contact op met AstraPlanner's privacy-contactpunt via `[INVULLEN: e-mailadres]`. Wij helpen je kosteloos met feitelijke vragen over onze verwerking. Voor juridische interpretatie van de AVG verwijzen wij je naar je eigen juridisch adviseur of Functionaris voor Gegevensbescherming.
