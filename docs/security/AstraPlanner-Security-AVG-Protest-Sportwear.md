...
### 10.3 Wat wij eerlijk benoemen als beperking

**Actor_id bij service-role acties (hardening vereist).** Wanneer AstraPlanner's AI-laag of een administratief endpoint wijzigingen aanbrengt via de service-role verbinding, is het mogelijk dat het `actor_id`-veld in de audit-log op `NULL` staat in plaats van de werkelijke gebruiker. Dit komt doordat de JWT-context niet altijd door de service-role verbinding wordt doorgegeven aan de trigger. Wij gaan dit vóór go-live volledig oplossen door de actor expliciet mee te geven bij elke admin-client mutatie. Dit punt wordt ook contractueel opgenomen in onze hardening-garantie (§15).

**Administratieve endpoints.** Het toekennen van gebruikers aan een organisatie (`assign-org`) werkt met Supabase's `auth.admin` API en wordt momenteel niet expliciet naar `audit_log` geschreven. Dit wordt eveneens vóór go-live opgelost.
