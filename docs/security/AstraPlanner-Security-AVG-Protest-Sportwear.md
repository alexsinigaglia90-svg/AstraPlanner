...
| 4 | **Security headers** | ✅ **Gerealiseerd** — `next.config.ts` levert nu Content-Security-Policy, Strict-Transport-Security (2 jaar, includeSubDomains), X-Frame-Options: DENY, X-Content-Type-Options: nosniff, Referrer-Policy: strict-origin-when-cross-origin, Permissions-Policy, en Cross-Origin-Opener-Policy op elke route. Aanvullend draait sinds de hardening wave een **strict-dynamic CSP met per-request nonces** in `Content-Security-Policy-Report-Only` modus (via `src/proxy.ts`); na een observatieperiode wordt deze policy enforced en vervangt hij de huidige enforced policy, waarmee `'unsafe-inline'` voor scripts volledig wordt verwijderd | — | ✅ Gereed (enforced via next.config.ts + report-only strict-dynamic via middleware) |
